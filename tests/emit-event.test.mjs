...
test('emit appends a schema-complete JSONL line', () => {
  const before = fs.existsSync(monthFile()) ? fs.readFileSync(monthFile(), 'utf8').split('\n').filter(Boolean).length : 0;
  const e = emit({ phase: 'build', event: 'gate_run', agent_role: 'builder', result: 'pass', task_id: 'TEST-1', detail: { gate: 'ci' } });
  ...
  for (const key of ['ts', 'run_id', 'task_id', 'phase', 'event', 'agent_role', 'model', 'tokens_in', 'tokens_out', 'cost_usd', 'duration_ms', 'result', 'detail']) {
