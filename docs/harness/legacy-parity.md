...
| 6. Legacy shims → removal | ✅ **COMPLETE** (6a 2026-07-06 CHG-0020; 6b 2026-07-16, Tim-directed). 6a: all five legacy files (`bin/gate-{ci,verify,perf}.mjs`, `bin/harness-resolve.mjs`, `lib/harness.mjs`) replaced with one-line forwarding shims into `harness/gates/`, verified byte-identical behaviour. 6b after ~10 days of quiet shims (M2 shipped entirely through the new paths): `~/.claude/{bin,lib}` deleted including the `.legacy-originals-2026-07-06/` backup; old-path prose swept from `~/.claude/{HARNESS.md, agents/planner.md, agents/tdd.md, skills/sdlc/SKILL.md}` (repo grep confirmed zero live references first); resolver re-verified from the canonical path post-delete. The legacy layer no longer exists — `harness/gates/` is the only implementation. |
