...
- [x] 1.2 PR to this repo; `harness-config-approved` label from Tim
      (.github/** protected); read the actual nested check contexts off the
      PR run, update `main-protection` required checks
      (`semgrep`→`security / semgrep` etc.), merge. (PR #27 merged 7db6303;
      contexts confirmed `security / semgrep|gitleaks|trivy`; ruleset
...
- [x] 1.4 nrl-predictor caller `.github/workflows/security.yml`
      ...
      cross-repo live drill. (nrl-predictor PR #10, all checks green,
      merged 8c31f4d.)
- [x] 1.5 Follow-up recorded: fan-out to kickpool/aitutor/remaining repos +
      per-repo required-check wiring once stable. Merge; archive bundle.
      (Merged; archived 2026-07-15.)
