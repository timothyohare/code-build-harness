# gate-fuzz Specification

## Purpose

Schema-driven API fuzz contract: `harness/gates/fuzz.mjs` boots the app
deterministically and runs Schemathesis against the repo's bound OpenAPI
document (`fuzzSchema`), failing on server errors and schema-violating
responses. On-demand like gate-perf — not in the Stop hook or per-PR
required checks. Established by CHG-0026; pilot nrl-predictor (first run
caught a live production crash on a non-integer path parameter).
